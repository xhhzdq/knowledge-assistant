...
| Docker Compose 四服务编排 | 待完成 | 第三周收尾目标 |
| 文档解析、切块 | 未开始 | 后续主线第一步 |
...
| 第 3 周 | 基础设施可重复运行 | MinIO、Redis、Docker、Compose | 对象存储、缓存、四服务 Compose |
| 第 4 周 | 文档内容可解析 | PDF/DOCX Parser、Chunk、OCR 基础 | 解析接口、Chunk 表、样例文档 |
| 第 5 周 | 文档片段可语义检索 | Embedding、Milvus | 向量入库、Search API、召回测试 |
| 第 6 周 | 检索结果更准确 | 关键词检索、混合检索、Reranker | 对比报告、检索评测集 |
| 第 7 周 | 支持实体关系查询 | Neo4j、实体关系抽取 | 图模型、Cypher 示例、图谱查询 API |
| 第 8 周 | 能基于证据回答 | RAG、Agno、引用 | Answers API、引用、幻觉测试 |
| 第 9 周 | 能被 Agent 和外部工具使用 | MCP、Skills、AI Coding 规范 | MCP Server、Skill 示例 |
| 第 10 周 | 可部署、可演示、可维护 | Nginx、监控、完整 Compose | 部署文档、演示脚本、总结 |
...
第三周先完成 Compose 收尾。随后第四周只做“解析与分块”闭环：
...
在这个闭环稳定前，不同时接入 Milvus、Neo4j 和 LLM。这样能够先验证解析质量、页码追溯和分块策略，为后续所有检索技术提供可靠输入。
